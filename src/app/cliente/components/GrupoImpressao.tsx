...
export function GrupoImpressao({
  ...
  const { codCliente, centro: empresa, linha: categoria } = grupo;
  const { dataRoteirizacao } = useRoteirizacaoStore();

  const infoTransporte = _.find(
    dataRoteirizacao,
    (find: RoteirizacaoItem) => find.cliente === codCliente
  );
  ...
  }, [codCliente, categoria, empresa, dadosAgrupado, configuracaoCD]);
