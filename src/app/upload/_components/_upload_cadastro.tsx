...
export default function CadastroProdutoPage() {
  const { dataCadastroItem, setDataCadastroItem } = useCadastroItemStore();
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await processExcelFileCadastroProduto(file);
  ...
  return (
    <div className="p-4 space-y-6 max-w-2xl mx-auto">
      <p className="text-muted-foreground">
        Faça upload do arquivo Excel de cadastro de produtos
      </p>
        ...
        <div className="p-4 border border-green-200 bg-green-50 rounded-md flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-green-600">
              {dataCadastroItem.length} itens foram carregados.
            </p>
          </div>
        </div>
      )}
